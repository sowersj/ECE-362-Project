...
#include "../sdcard/src/sdcard.c"
#include "../sdcard/src/diskio.c"
...
void keypad_isr() {
    ...
    for (int i = 2; i < 6; i++){
        ...
        if (gpio_get_irq_event_mask(i) == GPIO_IRQ_EDGE_RISE){
            ...
            int decrement = (int) key - 48
            if(decrement >= 0 && decrement <= 9) { // key must be a digit
                cat(sd_counter - decrement, hist_display);
            }
...
void timer_isr(){
    ...
    input(sd_counter, weather_block)
    sd_counter++;
