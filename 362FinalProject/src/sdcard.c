...
int input(char *filename, char *weatherdata)
{
    ...
    int len = strlen(weatherdata);
    UINT wlen;
    fr = f_write(&fil, (BYTE*)weatherdata, len, &wlen);
    if (fr) {
        f_close(&fil);
        return(1);
    }
...
int cat(char *filename, char *a, char *b, char *c)
{
    ...
    while (i < 3 && f_gets(line, sizeof(line), &fil)) {
        strncpy(numbers[i], line, 32);
        i++;
    }

    f_close(&fil);

    strncpy(a, numbers[0], 32);
    strncpy(b, numbers[1], 32);
    strncpy(c, numbers[2], 32);
